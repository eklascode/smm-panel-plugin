...
class api extends MX_Controller
{
    ...
    public function uddoktapay()
    {
        ...
        if (!empty($payload)) {
            ...
            $success = true;
            $apiKey = trim(get_option("uddoktapay_api_key"));
            ...
            $txnID = $data->metadata->transaction_id;
            ...
            if ($success) {
                ...
                $chagre_fee              = get_option('uddoktapay_chagre_fee');
                $amount = $data->amount / $this->convert;
                $tdata["transaction_id"]   = $transactionId;
                $tdata["status"]   = 1;
                $this->db->where('id', $txnID);
                $this->db->update($this->tb_transaction_logs, $tdata);

                /*----------  Add funds to user balance  ----------*/
                $user_balance   = get_field($this->tb_users, ["id" => $userID], "balance");

                $user_balance  += ($amount * 100) / (100 + $chagre_fee);

                $this->db->update($this->tb_users, ["balance" => $user_balance], ["id" => $userID]);
            }
        }
    }
